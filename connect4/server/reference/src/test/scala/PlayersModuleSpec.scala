...
trait TestableActorBasedPlayersModule extends ActorBasedPlayersModule {
  ...
  protected override def actorSystemFactory = new ActorSystemFactory {
    ...
    val unknownGame = TestActorRef(new UnknownGameActor())

    def createNewGameActor(id: Long) = {
      val actor = TestActorRef(new Connect4Actor(id))
      games.put(id, actor)
      actor
    }

    def lookupGame(id: Long) = games.get(id) getOrElse unknownGame
  }
